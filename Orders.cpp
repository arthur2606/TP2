...
using namespace std;
		...
		Orders::Orders(Client client,std::vector<Products> products,std::string statut):m_client(client),m_statut(statut)
		{
		...
		std::ostream &operator << (std::ostream &output,Orders& O) 
				{  
					output <<O.m_client->getPrenom()<<"  "<<O.m_client->getNom()<<"    "<<O.m_products->getTitle()<<"  "<<O.getStatut() <<"   "<<std::endl; 
					return output; 
